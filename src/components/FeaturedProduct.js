  ...
  render() {
    ...
    return (
      <View style={s.featuredContainer} key={num * 4}>
        <Image style={s.featuredProductImage} source={
          assetPaths.shop.featuredProducts[this.props.imageNumber].image
          }/>
        ...
        {this.props.productDots.map(dot => {
          num += 1;
          return (
            <View key={num * 2} style={[s.bubbleView, { top: dot.top, left: dot.left }]} >
              <TouchableOpacity onPress={() => this.showBubbles()}>
                <Image style={s.productDot} source={assetPaths.shop.icons.dot} />
              </TouchableOpacity>
              <Image
                style={[ s.productBubble,
                  { top: dot.bubbleOffset.top, left: dot.bubbleOffset.left },
                  this.state.bubbleShowing ? { opacity: bubbleOpacity } : { opacity: 0 }]}
                  source={dot.productBubble}/>
            </View>);
        })}
      </View>
    );
